...
PERSON get_record() {
    ...
    getchar();
    printf("이름: ");
    gets_s(data.name, SIZE); // 이름을 입력받는다
    printf("주소: ");
    gets_s(data.address, SIZE); // 주소를 입력받는다
    printf("휴대폰: ");
    gets_s(data.mobilephone, SIZE); // 휴대폰 번호를 입력받는다
    printf("특징: ");
    gets_s(data.desc, SIZE); // 특징을 입력 받는다

    //printf("%s %s %s %s", data.name, data.address, data.mobilephone, data.desc);
    return data;
}
...
void update_record(FILE* fp) {
    ...
    if ((fp1 = fopen("tmp.dat", "w")) == NULL) {
        fprintf(stderr, "파일 tem.dat를 열 수 없습니다");
        exit(1);
    }
    fseek(fp, 0, SEEK_SET);
    fflush(stdin);
    ...
    while (fread(&data, sizeof(data), 1, fp) == 1) {
        if (strcmp(data.name, name) == 0)
            data = get_record(); //수정하는게 되어야한다. 받는게 아니고
